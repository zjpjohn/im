package com.shiku.lbs.service.impl;
...
public class DefaultNearbyManagerImpl implements NearbyManager {
	...
	@Override
	public List<Integer> getCompanyIdList(BasePoi poi) {
		List<Integer> idList = Lists.newArrayList();
		List<CompanyVO> companyList = getCompanyList(poi);
	...
	@Override
	public List<CompanyVO> getCompanyList(BasePoi poi) {
		List<CompanyVO> poiList = Lists.newArrayList();
		...
		try {
			...
			if (0 == result.getStatus())
				poiList = JSON
						.parseArray(result.getContents(), CompanyVO.class);
		} catch (Exception e) {
			e.printStackTrace();
		}
