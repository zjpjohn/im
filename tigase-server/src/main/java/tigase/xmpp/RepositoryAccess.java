...
public abstract class RepositoryAccess {
    ...
    public void addDataList(final String subnode, final String key, final String[] list)
        ...
        try {
            this.repo.addDataList(getBareJID(), subnode, key, list);
    ...
    public Authorization loginOther(Map<String, Object> props)
        ...
        try {
            ...
            if (this.domain.isAnonymousEnabled() && (mech != null) && mech.equals(ANONYMOUS_MECH)) {
                ...
                if (this.authRepo.otherAuth(props)) {
                    this.authState = Authorization.AUTHORIZED;
                    login();
                } // end of if (authRepo.loginPlain())auth.login();
            }
    ...
    public void queryAuth(Map<String, Object> authProps) throws TigaseDBException {
        ...
        if (this.domain.isAnonymousEnabled()
    ...
    public Authorization register(String name_param, String pass_param, Map<String, String> reg_params)
        ...
        // paramProperties papos = new paramProperties();
        // String new_xmpp_domain = papos.getXmpp_domain();
        String new_xmpp_domain = "htigase.server.c3cloud.com.cn";
        log.log(Level.WARNING, "获取配置信息是：" + new_xmpp_domain);
        if (!this.domain.isRegisterEnabled()) {
            throw new NotAuthorizedException("Registration is now allowed for this domain");
        }
        if (this.domain.getMaxUsersNumber() > 0) {
            long domainUsers = this.authRepo.getUsersCount(this.domain.getVhost().getDomain());

            if (log.isLoggable(Level.FINEST)) {
                log.finest("Current number of users for domain: " + this.domain.getVhost()
        ...
        try {
            log.log(Level.WARNING, "******注册的用户信息是：" + user_name + "，|||" + pass_param + "，|||" + new_xmpp_domain);
            this.authRepo.addUser(BareJID.bareJIDInstance(user_name, new_xmpp_domain), pass_param);
            if (log.isLoggable(Level.FINEST)) {
                log.log(Level.FINEST, "User added: {0}, pass: {1}",
                        new Object[] { BareJID.toString(user_name, new_xmpp_domain), pass_param });
            }
            setRegistration(user_name, pass_param, reg_params);
            if (log.isLoggable(Level.FINEST)) {
                log.log(Level.FINEST, "Registration data set for: {0}, pass: {1}, reg_params: {2}",
                        new Object[] { BareJID.toString(user_name, new_xmpp_domain), pass_param, reg_params });
            }
    ...
    public void removeData(String subnode, String key)
            throws NotAuthorizedException, TigaseDBException {
        try {
            this.repo.removeData(getBareJID(), subnode, key);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public void removeDataGroup(final String subnode)
        ...
        try {
            this.repo.removeSubnode(getBareJID(), subnode);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public Authorization unregister(String name_param)
        ...
        if (getUserName().equals(user_name)) {
            try {
                this.authRepo.removeUser(BareJID.bareJIDInstance(user_name, getDomain().getVhost()
                        .getDomain()));
                try {
                    this.repo.removeUser(BareJID.bareJIDInstance(user_name, getDomain().getVhost()
                            .getDomain()));
    ...
    public String getData(String subnode, String key, String def)
        ...
        try {
            return this.repo.getData(getBareJID(), subnode, key, def);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public String[] getDataGroups(String subnode)
        ...
        try {
            return this.repo.getSubnodes(getBareJID(), subnode);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public String[] getDataKeys(final String subnode)
        ...
        try {
            return this.repo.getKeys(getBareJID(), subnode);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public String[] getDataList(String subnode, String key)
        ...
        try {
            return this.repo.getDataList(getBareJID(), subnode, key);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public void setData(String subnode, String key, String value)
            throws NotAuthorizedException, TigaseDBException {
        try {
            this.repo.setData(getBareJID(), subnode, key, value);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    public void setDataList(final String subnode, final String key, final String[] list)
        ...
        try {
            this.repo.setDataList(getBareJID(), subnode, key, list);
        } catch (UserNotFoundException e) {
            log.log(Level.FINEST, "Problem accessing reposiotry: ", e);
    ...
    private void setRegistration(final String name_param, final String pass_param,
        ...
        try {
            String new_xmpp_domain = "10.0.1.124";
            log.log(Level.WARNING,
                    "*****getDomain().getVhost().getDomain()：" + getDomain().getVhost().getDomain() + "******");
            log.log(Level.WARNING, "*****new_xmpp_domain：" + new_xmpp_domain + "******");
            log.log(Level.WARNING, "*****registr_params：" + registr_params + "******");
            this.authRepo.updatePassword(BareJID.bareJIDInstance(name_param, new_xmpp_domain), pass_param);
            if (registr_params != null) {
                for (Map.Entry<String, String> entry : registr_params.entrySet()) {
                    this.repo.setData(BareJID.bareJIDInstance(name_param, new_xmpp_domain), entry.getKey(),
                            entry.getValue());
                }
            }
        } catch (UserNotFoundException e) {
