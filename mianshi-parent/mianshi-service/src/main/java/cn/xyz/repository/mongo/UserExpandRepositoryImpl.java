...
@Service
public class UserExpandRepositoryImpl extends MongoRepository implements UserExpandRepository {
	...
	@Override
	public List<ThridPartyAccount> findAccounts(int userId) {
		User user = dsForRW.createQuery(User.class).field(Mapper.ID_KEY).equal(userId).get();

		return null == user ? Lists.newArrayList() : user.getAccounts();
	}
